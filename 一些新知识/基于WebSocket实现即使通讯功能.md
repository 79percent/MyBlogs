# 基于 WebSocket 实现即时通讯功能

最终效果  
![即时通讯功能DEMO](./img/wsDemo.gif)
[仓库地址](https://github.com/79percent/chat-websocket)  
服务器端语言选用了 Node.js, 使用了[nodejs-websocket](https://www.npmjs.com/package/nodejs-websocket)包。  
**nodejs-websocket**是用于 websocket 服务器和客户端的 Node.js 模块。
  ...
  .createServer((conn) => {
    conn.on("text", (str) => {
      ...
      if (connInex === -1) {
        ...
        if (
          userId === queue[connInex].userId &&
          targetId === queue[connInex].targetId
        ) {
        ...
        if (
          userId === queue[connInex].targetId &&
          targetId === queue[connInex].userId
        ) {
...
这个 DEMO 可以实现多个用户同时通信，但是只能进行在线通信，也就是说没有接收离线消息这个功能。
